...
def login(page, account, settings, mfa_provider, evidence_dir=None):
    t = settings.timeouts
    ...
    _fill_credentials(page, account, t)
    ...
    state = _wait_any(page, states, t.default_ms)
    ...
    _enter_code_loop(page, t, mfa_provider, evidence_dir)
...
def reauthenticate(page, account, settings, mfa_provider, evidence_dir=None):
    ...
    state = _wait_any(page, entry, t.default_ms)
    if state in ("dashboard", "tile"):
    ...
    if state == "email_select":
        ...
        state = _wait_any(page, {k: v for k, v in entry.items()
                                 if k not in ("username", "email_select")}, t.default_ms)
    if state == "error":
        raise LoginError(f"Okta rejected re-auth: {_error_text(page)}")
    if state in ("dashboard", "tile"):
...
def _fill_credentials(page, account, t):
...
def _ensure_code_entry_ui(page, t):
...
def _enter_code_loop(page, t, mfa_provider, evidence_dir=None):
...
def _wait_any(page, named_selectors, timeout_ms):
